...
@router.post(
    ...
    description="Receives transcript text and returns a concise summary and extracted action items using Groq LLM.",
    tags=["LLM Features"],
    dependencies=[Depends(check_llm_availability)] # Protect endpoint if LLM not ready
)
async def summarize_endpoint(request: SummarizationRequest):
    ...
    try:
        ...
        result = await llm_service.summarize_transcript(request.transcript)
        return result
        ...
        logger.exception("Unhandled exception during summarization") # Log full trace
...
async def chat_endpoint(request: ChatRequest):
    ...
    try:
        logger.info(f"Received chat query: '{request.user_query[:50]}...'") # Log snippet
